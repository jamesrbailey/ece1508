...
int main(int argc, char** argv) {
    ...
    unsigned int first_error_line = 0;
    while (getline(cin, line))
    {
        ...
        if(bit_errors == 0) {
            while (iss >> rx_sym) {
                if(rx_sym == "e") {
                    g.vars[i]->value = ERASE;
                    DEBUG(cout << "erase" << endl);
                }else if(rx_sym == "1") {
                    g.vars[i]->value = ONE;
                    DEBUG(cout << "one" << endl);
                }else if(rx_sym == "0") {
                    g.vars[i]->value = ZERO;
                    DEBUG(cout << "zero" << endl);
                } else {
                    cerr << "invalid symbol on stdin" << endl;
                }
                ++i;
                //cout << i << endl;
            } 

            bit_errors = g.decode(10);
            first_error_line = line_num;
        } 
    }
    unsigned int delta = line_num - first_error_line;
    cout << delta << endl;
    //g.vars = 
